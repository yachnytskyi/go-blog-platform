...
func extractAccessToken(ginContext *gin.Context, logger applicationModel.Logger, location string) commonModel.Result[string] {
	...
	logger.Error(httpAuthorizationError)
...
func extractRefreshToken(ginContext *gin.Context, logger applicationModel.Logger) commonModel.Result[string] {
...
func abortWithStatusJSON(ginContext *gin.Context, logger applicationModel.Logger, err error, httpCode int) {
	logger.Error(err)
